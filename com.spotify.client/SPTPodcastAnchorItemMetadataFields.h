//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "SPTCollectionPlatformPodcastMetadataFields-Protocol.h"

@class NSDate, NSDictionary, NSString, NSURL;

@interface SPTPodcastAnchorItemMetadataFields : NSObject <SPTCollectionPlatformPodcastMetadataFields>
{
}

@property(readonly, copy, nonatomic) NSString *publisher;
@property(readonly, nonatomic) NSURL *URL;
@property(readonly, nonatomic) NSDictionary *podcastCovers;
@property(readonly, copy, nonatomic) NSString *name;
@property(readonly, nonatomic) unsigned long long mediaType;
@property(readonly, nonatomic) NSDate *latestPublishedEpisodeDate;
@property(readonly, nonatomic) _Bool hasNewEpisodes;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

