//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSObject.h>

#import "SPTBannerViewDelegate-Protocol.h"

@class NSString, SPTGoogleMapsLogger;
@protocol GLUETheme, SPTBannerPresentationManager, SPTBannerPresentationManagerTicket, SPTLinkDispatcher;

@interface SPTGoogleMapsPresenter : NSObject <SPTBannerViewDelegate>
{
    id <SPTBannerPresentationManagerTicket> _bannerTicket;
    id <SPTBannerPresentationManager> _bannerPresentationManager;
    id <GLUETheme> _glueTheme;
    id <SPTLinkDispatcher> _linkDispatcher;
    SPTGoogleMapsLogger *_logger;
}

@property(readonly, nonatomic) __weak SPTGoogleMapsLogger *logger; // @synthesize logger=_logger;
@property(readonly, nonatomic) __weak id <SPTLinkDispatcher> linkDispatcher; // @synthesize linkDispatcher=_linkDispatcher;
@property(readonly, nonatomic) __weak id <GLUETheme> glueTheme; // @synthesize glueTheme=_glueTheme;
@property(readonly, nonatomic) __weak id <SPTBannerPresentationManager> bannerPresentationManager; // @synthesize bannerPresentationManager=_bannerPresentationManager;
@property(retain, nonatomic) id <SPTBannerPresentationManagerTicket> bannerTicket; // @synthesize bannerTicket=_bannerTicket;
- (void).cxx_destruct;
- (void)closeButtonTappedInBannerView:(id)arg1;
- (void)actionButtonTappedInBannerView:(id)arg1;
- (void)hideBanner;
- (void)showBanner;
- (id)initWithBannerPresentationManager:(id)arg1 linkDispatcher:(id)arg2 glueTheme:(id)arg3 logger:(id)arg4;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

