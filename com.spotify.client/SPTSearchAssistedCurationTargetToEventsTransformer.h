//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSObject.h>

#import "SPTSearchTargetToEventsTransforming-Protocol.h"

@class NSString;

@interface SPTSearchAssistedCurationTargetToEventsTransformer : NSObject <SPTSearchTargetToEventsTransforming>
{
}

- (void)adjustSearchResultsRowComponentID:(id)arg1;
- (id)uriByAddingAssistedCurationEntityPrefixToURI:(id)arg1;
- (void)addEntityDrillDownCommandModelInBodyComponentModelBuilder:(id)arg1;
- (id)uriByAddingAssistedCurationPrefixToURI:(id)arg1;
- (void)addDrillDownNavigationCommandModelsInComponentModelBuilders:(id)arg1;
- (void)addTrackRowCommandModelInBodyCompoenentModelBuilder:(id)arg1;
- (void)applyTransformationToViewModelBuilder:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

