//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "SPTPerformanceMetricsTransport-Protocol.h"
#import "SPTPerformanceMetricsTransportRegistry-Protocol.h"

@class NSSet, NSString;

@interface SPTPerformanceMetricsTransports : NSObject <SPTPerformanceMetricsTransport, SPTPerformanceMetricsTransportRegistry>
{
    NSSet *_transports;
}

+ (id)defaultTransportsWithLogCenter:(id)arg1 startupTracer:(id)arg2;
@property(copy, nonatomic) NSSet *transports; // @synthesize transports=_transports;
- (void).cxx_destruct;
- (void)removeTransport:(id)arg1;
- (void)addTransport:(id)arg1;
- (void)willSendForMessageClass:(Class)arg1;
- (void)send:(id)arg1;
- (id)initWithTransports:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

