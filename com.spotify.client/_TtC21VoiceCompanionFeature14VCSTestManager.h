//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@interface _TtC21VoiceCompanionFeature14VCSTestManager : NSObject
{
    // Error parsing type: , name: delegate
    // Error parsing type: , name: experiment
    // Error parsing type: , name: entryPoint
    // Error parsing type: , name: fullscreenEnabledSignal
    // Error parsing type: , name: fullscreenEntryPointAlwaysVisibleSignal
    // Error parsing type: , name: snackbarEnabledSignal
    // Error parsing type: , name: snackbarEntryPointAlwaysVisibleSignal
    // Error parsing type: , name: featureFlagFactory
}

- (void).cxx_destruct;
- (id)init;

@end

