//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class NSMutableArray, YTIFormattedString, YTIResponseContext;

@interface YTIPlaceAutocompleteResponse : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(nonatomic) _Bool hasNoResultsLabel; // @dynamic hasNoResultsLabel;
@property(nonatomic) _Bool hasResponseContext; // @dynamic hasResponseContext;
@property(retain, nonatomic) YTIFormattedString *noResultsLabel; // @dynamic noResultsLabel;
@property(retain, nonatomic) NSMutableArray *placeSuggestionsArray; // @dynamic placeSuggestionsArray;
@property(readonly, nonatomic) unsigned long long placeSuggestionsArray_Count; // @dynamic placeSuggestionsArray_Count;
@property(retain, nonatomic) YTIResponseContext *responseContext; // @dynamic responseContext;

@end

