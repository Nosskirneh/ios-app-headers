//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "YTSectionController.h"

@protocol YTInnerTubeUIService;

@interface YTServiceSectionController : YTSectionController
{
    id <YTInnerTubeUIService> _service;
}

@property(readonly, nonatomic) id <YTInnerTubeUIService> service; // @synthesize service=_service;
- (void).cxx_destruct;
- (id)initWithService:(id)arg1 parentResponder:(id)arg2 builder:(id)arg3;
- (id)initWithCellFactory:(id)arg1 service:(id)arg2 parentResponder:(id)arg3 builder:(id)arg4;

@end

