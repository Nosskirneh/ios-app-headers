//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class NSString, YTIOfflineErrorMetadata;

@interface YTIOfflineSend : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(nonatomic) int detailedNetworkType; // @dynamic detailedNetworkType;
@property(nonatomic) int discoCode; // @dynamic discoCode;
@property(retain, nonatomic) YTIOfflineErrorMetadata *errorData; // @dynamic errorData;
@property(nonatomic) int event; // @dynamic event;
@property(nonatomic) _Bool hasDetailedNetworkType; // @dynamic hasDetailedNetworkType;
@property(nonatomic) _Bool hasDiscoCode; // @dynamic hasDiscoCode;
@property(nonatomic) _Bool hasErrorData; // @dynamic hasErrorData;
@property(nonatomic) _Bool hasEvent; // @dynamic hasEvent;
@property(nonatomic) _Bool hasOfflineShareSessionNonce; // @dynamic hasOfflineShareSessionNonce;
@property(nonatomic) _Bool hasOfflineTransferVersion; // @dynamic hasOfflineTransferVersion;
@property(nonatomic) _Bool hasShareNonce; // @dynamic hasShareNonce;
@property(copy, nonatomic) NSString *offlineShareSessionNonce; // @dynamic offlineShareSessionNonce;
@property(nonatomic) int offlineTransferVersion; // @dynamic offlineTransferVersion;
@property(copy, nonatomic) NSString *shareNonce; // @dynamic shareNonce;

@end

