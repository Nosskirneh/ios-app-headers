//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class GPBDoubleArray, GPBInt64Array, NSMutableArray, YTICommand, YTIFormattedString;

@interface YTIAnalyticsTimeSeriesCardRenderer : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(retain, nonatomic) GPBInt64Array *datesArray; // @dynamic datesArray;
@property(readonly, nonatomic) unsigned long long datesArray_Count; // @dynamic datesArray_Count;
@property(retain, nonatomic) YTICommand *detailsEndpoint; // @dynamic detailsEndpoint;
@property(retain, nonatomic) YTIFormattedString *displayShortTotal; // @dynamic displayShortTotal;
@property(retain, nonatomic) YTIFormattedString *displayTotal; // @dynamic displayTotal;
@property(nonatomic) unsigned int graphColor; // @dynamic graphColor;
@property(nonatomic) _Bool hasDetailsEndpoint; // @dynamic hasDetailsEndpoint;
@property(nonatomic) _Bool hasDisplayShortTotal; // @dynamic hasDisplayShortTotal;
@property(nonatomic) _Bool hasDisplayTotal; // @dynamic hasDisplayTotal;
@property(nonatomic) _Bool hasGraphColor; // @dynamic hasGraphColor;
@property(nonatomic) _Bool hasSubtitle; // @dynamic hasSubtitle;
@property(nonatomic) _Bool hasTitle; // @dynamic hasTitle;
@property(nonatomic) _Bool hasTitleDescription; // @dynamic hasTitleDescription;
@property(nonatomic) _Bool hasTotal; // @dynamic hasTotal;
@property(retain, nonatomic) YTIFormattedString *subtitle; // @dynamic subtitle;
@property(retain, nonatomic) YTIFormattedString *title; // @dynamic title;
@property(retain, nonatomic) YTIFormattedString *titleDescription; // @dynamic titleDescription;
@property(nonatomic) double total; // @dynamic total;
@property(retain, nonatomic) GPBDoubleArray *valuesArray; // @dynamic valuesArray;
@property(readonly, nonatomic) unsigned long long valuesArray_Count; // @dynamic valuesArray_Count;
@property(retain, nonatomic) NSMutableArray *xAxisLabelsArray; // @dynamic xAxisLabelsArray;
@property(readonly, nonatomic) unsigned long long xAxisLabelsArray_Count; // @dynamic xAxisLabelsArray_Count;

@end

