//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "HAMDASHBaseElementParser.h"

@class HAMDASHMPDBuilder;

@interface HAMDASHMPDElementParser : HAMDASHBaseElementParser
{
    HAMDASHMPDBuilder *_builder;
}

+ (id)MPDBuilder;
@property(readonly, nonatomic) HAMDASHMPDBuilder *builder; // @synthesize builder=_builder;
- (void).cxx_destruct;
- (_Bool)endElementWithErrorDetails:(id)arg1;
- (_Bool)parseAttribute:(const char *)arg1 value:(const char *)arg2 valueLength:(unsigned long long)arg3;
- (id)startElementWithName:(const char *)arg1;
- (id)initWithFactory:(id)arg1;

@end

