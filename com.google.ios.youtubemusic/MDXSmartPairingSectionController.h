//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "MDXScreenManagerObserver-Protocol.h"
#import "MDXSmartPairingCellDelegate-Protocol.h"
#import "MDXSmartPairingDeviceCellDelegate-Protocol.h"
#import "MDXSmartPairingInputTVCodeCellDelegate-Protocol.h"
#import "YTSectionControllerProtocol-Protocol.h"

@class MDXScreenManager, NSArray, NSString, YTICommand, YTReachabilityController;
@protocol MDXSmartPairingSectionControllerDelegate, YTSectionControllerDelegate;

@interface MDXSmartPairingSectionController : NSObject <MDXSmartPairingCellDelegate, MDXSmartPairingDeviceCellDelegate, MDXSmartPairingInputTVCodeCellDelegate, MDXScreenManagerObserver, YTSectionControllerProtocol>
{
    NSArray *_items;
    YTReachabilityController *_reachabilityController;
    MDXScreenManager *_screenManager;
    YTICommand *_navEndpoint;
    id <YTSectionControllerDelegate> _sectionControllerDelegate;
    id <MDXSmartPairingSectionControllerDelegate> _delegate;
}

+ (id)sectionTitleForCellType:(long long)arg1;
@property(nonatomic) __weak id <MDXSmartPairingSectionControllerDelegate> delegate; // @synthesize delegate=_delegate;
@property(nonatomic) __weak id <YTSectionControllerDelegate> sectionControllerDelegate; // @synthesize sectionControllerDelegate=_sectionControllerDelegate;
- (void).cxx_destruct;
- (void)didTapCell:(id)arg1 withNavEndpoint:(id)arg2;
- (void)MDXSmartPairingDeviceCell:(id)arg1 willConnectToScreen:(id)arg2;
- (void)MDXSmartPairingDeviceCell:(id)arg1 shouldDeleteScreen:(id)arg2;
- (void)MDXSmartPairingDeviceCell:(id)arg1 shouldDisconectFromScreen:(id)arg2;
- (void)MDXSmartPairingDeviceCell:(id)arg1 shouldConnectToScreen:(id)arg2;
- (id)implementationForBehavior:(id)arg1;
- (void)setNavEndpoint:(id)arg1 forItem:(id)arg2;
- (void)setScreenManager:(id)arg1;
- (void)setReachabilityController:(id)arg1;
- (struct CGSize)sizeForHeaderWithSize:(struct CGSize)arg1;
- (void)updateHeader:(id)arg1 indexPath:(id)arg2;
- (Class)classForHeader;
- (void)updateCell:(id)arg1 forIndex:(long long)arg2;
- (struct CGSize)sizeForItemAtIndex:(long long)arg1 withSize:(struct CGSize)arg2;
- (id)cellClassesToRegister;
- (Class)cellClassForItemAtIndex:(long long)arg1;
- (_Bool)canLoadMore;
- (long long)numberOfItems;
- (id)model;
- (id)initWithItems:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

