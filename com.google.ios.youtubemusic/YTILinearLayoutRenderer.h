//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class NSMutableArray;

@interface YTILinearLayoutRenderer : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(nonatomic) _Bool hasLayoutStyle; // @dynamic hasLayoutStyle;
@property(nonatomic) _Bool hasMinSpaceBetweenItems; // @dynamic hasMinSpaceBetweenItems;
@property(nonatomic) _Bool hasOrientation; // @dynamic hasOrientation;
@property(retain, nonatomic) NSMutableArray *itemsArray; // @dynamic itemsArray;
@property(readonly, nonatomic) unsigned long long itemsArray_Count; // @dynamic itemsArray_Count;
@property(nonatomic) int layoutStyle; // @dynamic layoutStyle;
@property(nonatomic) float minSpaceBetweenItems; // @dynamic minSpaceBetweenItems;
@property(nonatomic) int orientation; // @dynamic orientation;

@end

