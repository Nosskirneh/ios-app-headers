//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class NSMutableArray, YTICommand, YTIFormattedString;

@interface YTINotificationTypeOptInRenderer : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(retain, nonatomic) NSMutableArray *clientSettingDependenciesArray; // @dynamic clientSettingDependenciesArray;
@property(readonly, nonatomic) unsigned long long clientSettingDependenciesArray_Count; // @dynamic clientSettingDependenciesArray_Count;
@property(nonatomic) _Bool hasHideEndpoint; // @dynamic hasHideEndpoint;
@property(nonatomic) _Bool hasIndex; // @dynamic hasIndex;
@property(nonatomic) _Bool hasShow; // @dynamic hasShow;
@property(nonatomic) _Bool hasSummary; // @dynamic hasSummary;
@property(nonatomic) _Bool hasTitle; // @dynamic hasTitle;
@property(nonatomic) _Bool hasToggleOnText; // @dynamic hasToggleOnText;
@property(retain, nonatomic) YTICommand *hideEndpoint; // @dynamic hideEndpoint;
@property(nonatomic) long long index; // @dynamic index;
@property(nonatomic) _Bool show; // @dynamic show;
@property(retain, nonatomic) YTIFormattedString *summary; // @dynamic summary;
@property(retain, nonatomic) YTIFormattedString *title; // @dynamic title;
@property(retain, nonatomic) YTIFormattedString *toggleOnText; // @dynamic toggleOnText;

@end

