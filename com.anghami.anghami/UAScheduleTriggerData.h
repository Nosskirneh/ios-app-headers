//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <CoreData/NSManagedObject.h>

@class NSData, NSDate, NSNumber, UAActionScheduleData, UAScheduleDelayData;

@interface UAScheduleTriggerData : NSManagedObject
{
}


// Remaining properties
@property(retain, nonatomic) UAScheduleDelayData *delay; // @dynamic delay;
@property(retain, nonatomic) NSNumber *goal; // @dynamic goal;
@property(retain, nonatomic) NSNumber *goalProgress; // @dynamic goalProgress;
@property(retain, nonatomic) NSData *predicateData; // @dynamic predicateData;
@property(retain, nonatomic) UAActionScheduleData *schedule; // @dynamic schedule;
@property(retain, nonatomic) NSDate *start; // @dynamic start;
@property(retain, nonatomic) NSNumber *type; // @dynamic type;
@end

