//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "NativeHomeCover.h"

@class NSLayoutConstraint, Photo, UIImageView, UILabel;

@interface NativeHomePhotoCover : NativeHomeCover
{
    NSLayoutConstraint *_artistLabelWidth;
    Photo *_anghamiPhoto;
    UILabel *_likesLabel;
    UIImageView *_likeImageView;
}

+ (id)placeholderImage;
+ (long long)numberOfLinesSubtitle;
+ (long long)numberOfLinesTitle;
@property(retain, nonatomic) UIImageView *likeImageView; // @synthesize likeImageView=_likeImageView;
@property(retain, nonatomic) UILabel *likesLabel; // @synthesize likesLabel=_likesLabel;
@property(retain, nonatomic) Photo *anghamiPhoto; // @synthesize anghamiPhoto=_anghamiPhoto;
- (void).cxx_destruct;
- (id)accessibilityLabel;
- (void)layoutTitleLabel;
- (void)setupSubviewConstraints;
- (void)addSubviews;

@end

