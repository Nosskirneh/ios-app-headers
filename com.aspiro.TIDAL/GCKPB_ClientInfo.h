//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GCKPB_PBGeneratedMessage.h"

@class GCKPB_AndroidClientInfo, GCKPB_BrowserInfo, GCKPB_DesktopClientInfo, GCKPB_IosClientInfo, GCKPB_JsClientInfo, GCKPB_PancettaClientInfo, GCKPB_PlayCeClientInfo, GCKPB_VrClientInfo, NSString;

@interface GCKPB_ClientInfo : GCKPB_PBGeneratedMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(retain, nonatomic) GCKPB_AndroidClientInfo *androidClientInfo; // @dynamic androidClientInfo;
@property(retain, nonatomic) GCKPB_BrowserInfo *browserInfo; // @dynamic browserInfo;
@property(nonatomic) int clientType; // @dynamic clientType;
@property(retain, nonatomic) GCKPB_DesktopClientInfo *desktopClientInfo; // @dynamic desktopClientInfo;
@property(nonatomic) _Bool hasAndroidClientInfo; // @dynamic hasAndroidClientInfo;
@property(nonatomic) _Bool hasBrowserInfo; // @dynamic hasBrowserInfo;
@property(nonatomic) _Bool hasClientType; // @dynamic hasClientType;
@property(nonatomic) _Bool hasDesktopClientInfo; // @dynamic hasDesktopClientInfo;
@property(nonatomic) _Bool hasIosClientInfo; // @dynamic hasIosClientInfo;
@property(nonatomic) _Bool hasJsClientInfo; // @dynamic hasJsClientInfo;
@property(nonatomic) _Bool hasPancettaClientInfo; // @dynamic hasPancettaClientInfo;
@property(nonatomic) _Bool hasPlayCeClientInfo; // @dynamic hasPlayCeClientInfo;
@property(nonatomic) _Bool hasRemoteHost; // @dynamic hasRemoteHost;
@property(nonatomic) _Bool hasRemoteHost6; // @dynamic hasRemoteHost6;
@property(nonatomic) _Bool hasVrClientInfo; // @dynamic hasVrClientInfo;
@property(retain, nonatomic) GCKPB_IosClientInfo *iosClientInfo; // @dynamic iosClientInfo;
@property(retain, nonatomic) GCKPB_JsClientInfo *jsClientInfo; // @dynamic jsClientInfo;
@property(retain, nonatomic) GCKPB_PancettaClientInfo *pancettaClientInfo; // @dynamic pancettaClientInfo;
@property(retain, nonatomic) GCKPB_PlayCeClientInfo *playCeClientInfo; // @dynamic playCeClientInfo;
@property(retain, nonatomic) NSString *remoteHost; // @dynamic remoteHost;
@property(retain, nonatomic) NSString *remoteHost6; // @dynamic remoteHost6;
@property(retain, nonatomic) GCKPB_VrClientInfo *vrClientInfo; // @dynamic vrClientInfo;

@end

