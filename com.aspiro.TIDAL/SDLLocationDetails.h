//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "SDLRPCStruct.h"

@class NSArray, NSString, SDLImage, SDLLocationCoordinate, SDLOasisAddress;

@interface SDLLocationDetails : SDLRPCStruct
{
}

@property(retain, nonatomic) SDLOasisAddress *searchAddress;
@property(retain, nonatomic) SDLImage *locationImage;
@property(copy, nonatomic) NSString *phoneNumber;
@property(copy, nonatomic) NSString *locationDescription;
@property(copy, nonatomic) NSArray *addressLines;
@property(copy, nonatomic) NSString *locationName;
@property(retain, nonatomic) SDLLocationCoordinate *coordinate;

@end

